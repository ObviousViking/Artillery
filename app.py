...
MEDIA_WALL_INTERVAL_PATH = os.path.join(CONFIG_DIR, "media_wall_interval.txt")
DEFAULT_WALL_INTERVAL = 60  # minutes

def get_media_wall_interval():
    try:
        with open(MEDIA_WALL_INTERVAL_PATH, "r") as f:
            return int(f.read().strip())
    except Exception:
        return DEFAULT_WALL_INTERVAL

def set_media_wall_interval(minutes):
    with open(MEDIA_WALL_INTERVAL_PATH, "w") as f:
        f.write(str(minutes))

@app.route("/mediawall/interval", methods=["GET", "POST"])
def media_wall_interval():
    if request.method == "POST":
        minutes = int(request.form.get("interval", DEFAULT_WALL_INTERVAL))
        set_media_wall_interval(minutes)
        return jsonify({"success": True, "interval": minutes})
    return jsonify({"interval": get_media_wall_interval()})

import time

MEDIA_WALL_LAST_REFRESH_PATH = os.path.join(CONFIG_DIR, "media_wall_last_refresh.txt")

def get_last_wall_refresh():
    try:
        with open(MEDIA_WALL_LAST_REFRESH_PATH, "r") as f:
            return float(f.read().strip())
    except Exception:
        return 0

def set_last_wall_refresh(ts=None):
    with open(MEDIA_WALL_LAST_REFRESH_PATH, "w") as f:
        f.write(str(ts or time.time()))

def should_refresh_wall():
    interval = get_media_wall_interval() * 60  # minutes to seconds
    last = get_last_wall_refresh()
